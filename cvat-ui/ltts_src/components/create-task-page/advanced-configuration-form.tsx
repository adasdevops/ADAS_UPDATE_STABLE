...
class AdvancedConfigurationForm extends React.PureComponent<Props> {
    ...
    private renderSegmentSize(): JSX.Element {
        ...
        console.log(files,'files', files.local.length);
        const maxSegmentNumber = files.local.length;
        let maxSegmentLimit;
        const segmentValidator = { min: 1, max: undefined };
        if (projectType === 'Image') {
            segmentValidator.max = maxSegmentNumber;
            maxSegmentLimit = maxSegmentNumber;
        }
        return (
            <CVATTooltip title='Defines a number of frames in a segment'>
                <Form.Item label='Segment size' name='segmentSize' rules={[{ validator: isInteger(segmentValidator) }]}>
                    <Input size='large' type='number' max={maxSegmentLimit} placeholder='fileLength' />
                </Form.Item>
            </CVATTooltip>
        );
    }
