...
import UserSelector, { User } from '../../../src/components/task-page/user-selector';
...
function ReviewSummaryComponent({ jobInstance }: { jobInstance: any }): JSX.Element {
    ...
    useEffect(() => {
        setError(null);
        //jobInstance?.reviewsSummary()
        jobInstance.issues(jobInstance.id)
...
console.log('useruseruser',user);
    ...
    const jobsFilter = user.isSuperuser
        ...
        : jobs.filter(
              (item: any) =>
                  (item.assignee !== null && item.assignee.id === assigneeId)
                //   (item.reviewer !== null && item.reviewer.id === assigneeId),
          );
