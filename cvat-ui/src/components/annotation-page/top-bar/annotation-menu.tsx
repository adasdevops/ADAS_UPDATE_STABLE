...
function AnnotationMenuComponent(props: Props & RouteComponentProps): JSX.Element {
    ...
    return (
        <Menu onClick={(params: MenuInfo) => onClickMenuWrapper(params)} className='cvat-annotation-menu' selectable={false}>
            ...
            {role === 'worker' && <Menu.Item key={Actions.REMOVE_ANNO}>Remove annotations</Menu.Item>}
            ...
            {[JobStage.ANNOTATION, JobStage.REVIEW].includes(jobStage) && role === 'admin' ?
            ...
            {role === 'worker' && (
             ...
             {role === 'worker' && <Menu.Item key={Actions.FINISH_JOB}>Finish the job</Menu.Item>}
