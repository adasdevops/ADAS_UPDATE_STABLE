...
export enum AnnotationActionTypes {
    ...
    //#LTTS changes
    PROPAGATE_ALL_OBJECT = 'PROPAGATE_ALL_OBJECT',
    PROPAGATE_OBJECT_SUCCESS = 'PROPAGATE_OBJECT_SUCCESS',
    //#LTTS changes
    PROPAGATE_ALL_OBJECT_SUCCESS = 'PROPAGATE_ALL_OBJECT_SUCCESS',
...
// propagateAllObjectAsync added by giti
export function propagateAllObjectAsync(sessionInstance: any, objectState: any, from: number, to: number): ThunkAction {
    return async (dispatch: ActionCreator<Dispatch>): Promise<void> => {
        try {
            let copy;
            const copy1 = [];
            for (let i = 0; i < objectState.length; i++) {
                copy = {
                    attributes: objectState[i].attributes,
                    points: objectState[i].points,
                    occluded: objectState[i].occluded,
                    objectType: objectState[i].objectType !== ObjectType.TRACK ?
                        objectState[i].objectType : ObjectType.SHAPE,
                    shapeType: objectState[i].shapeType,
                    label: objectState[i].label,
                    zOrder: objectState[i].zOrder,
                    frame: from,
                    source: objectState[i].source,
                };
                copy1.push(copy);
            }
            ...
            for (let frame = from; frame <= to; frame++) {
                for (let i = 0; i < copy1.length; i++) {
                    copy1[i].frame = frame;
                    const newState = new cvat.classes.ObjectState(copy1[i]);
                    states.push(newState);
                }
            }
...
//LTTS changes below funcn
export function propagateAllObject(objectState: any | null): AnyAction {
    return {
        type: AnnotationActionTypes.PROPAGATE_ALL_OBJECT,
        payload: {
            objectState,
        },
    };
