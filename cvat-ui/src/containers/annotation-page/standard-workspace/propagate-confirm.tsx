...
import {
     // propagateObject as propagateObjectAction,#LTTS changes
     propagateAllObject as propagateAllObjectAction,
...
interface DispatchToProps {
    ...
    //LTTS changes
    propagateAllObject(sessionInstance: any, objectState: any, from: number, to: number): void;
...
function mapDispatchToProps(dispatch: any): DispatchToProps {
    return {
        ...
        //LTTS changes
        propagateAllObject(sessionInstance: any, objectStates: any, from: number, to: number): void {
            dispatch(propagateAllObjectAsync(sessionInstance, objectStates, from, to));
        },
        changePropagateFrames(frames: number): void {
        ...
        cancel(): void {
           ...
           //LTTS changes
            dispatch(propagateAllObjectAction(null));
        },
    };
}
...
class PropagateConfirmContainer extends React.PureComponent<Props> {
    ...
    //LTTS changes
    private propagateAllObject = (): void => {
        const {
            propagateAllObject, objectState, propagateFrames, frameNumber, stopFrame, jobInstance,
        } = this.props;

        const propagateUpToFrame = Math.min(frameNumber + propagateFrames, stopFrame);
        propagateAllObject(jobInstance, objectState, frameNumber + 1, propagateUpToFrame);
    };
    ...
    public render(): JSX.Element {
        ...
        return (
            <PropagateConfirmComponent
                ...
                //LTTS changes
                propagateAllObject={this.propagateAllObject}
