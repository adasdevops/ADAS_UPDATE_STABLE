...
import {
    ...
    propagateAllObject as propagateAllObjectAction,//LTTS changes
...
interface DispatchToProps {
    ...
    propagateAllObject: (objectState: any) => void;//LTTS changes
...
function mapDispatchToProps(dispatch: any): DispatchToProps {
    return {
        ...
        propagateAllObject(objectState: any): void {
            dispatch(propagateAllObjectAction(objectState));
        },
        changeFrame(frame: number): void {
...
class ObjectsListContainer extends React.PureComponent<Props, State> {
    ...
    public render(): JSX.Element {
        const {
            ...
            copyShape,
            propagateObject,
            propagateAllObject,//LTTS changes
            changeFrame,
        } = this.props;
        ...
        const subKeyMap = {
            ...
            PROPAGATE_ALL_OBJECT: keyMap.PROPAGATE_ALL_OBJECT,
            //LTTS changes
        ...
        //LTTS changes
        const activatedStatedAll = (): any | null => {
            if (objectStates !== null && objectStates.length > 1) {
                return objectStates;
            }
            return null;
        };
        ...
        const handlers = {
            ...
            //LTTS changes
            PROPAGATE_ALL_OBJECT: (event: KeyboardEvent | undefined) => {
                preventDefault(event);
                const state = activatedStatedAll();
                if (state && !readonly) {
                    propagateAllObject(state);
                }
            },
